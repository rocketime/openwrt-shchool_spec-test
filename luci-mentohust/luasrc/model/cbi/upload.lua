...
sul = ful:section(SimpleSection, "", translate("Upload file to '/etc/upload/'"))
sul = ful:section(SimpleSection, "", translate("fuck"))
...
http.setfilehandler(
	function(meta, chunk, eof)
		...
		if eof and fd then
			fd:close()
			fd = nil
			um.value = translate("File saved to") .. ' "/etc/upload/' .. meta.file .. '"'
		end
	end
)
...
local files = fs.dir("/etc/upload/")
if files then
	for f in files do
		attr = fs.stat("/etc/upload/" .. f)
		if attr then
			inits[i] = {}
			inits[i].name = f
...
btnrm.write = function(self, section)
	local v = fs.unlink("/etc/upload/" .. fs.basename(inits[section].name))
...
btnis.write = function(self, section)
	local r = luci.sys.exec(string.format('opkg --force-depends install "/etc/upload/%s"', inits[section].name))
